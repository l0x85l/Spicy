...
public class Bandana extends Cosmetic {
    ...
    public void render(AbstractClientPlayer player, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float headYaw, float headPitch, float scale) {
        User user = Spicy.getINSTANCE().getUser(player.getNameClear());
        if(user.hasBandana() && Minecraft.getMinecraft().thePlayer.inventory.armorInventory[3] == null) {
            GL11.glPushMatrix();
            playerRenderer.bindTexture(Spicy.getINSTANCE().cosmeticTextures.NIKE_BANDANA);
