package com.spicy.cosmetic.renderer;

import com.spicy.Spicy;
import com.spicy.cosmetic.Cosmetic;
import com.spicy.cosmetic.CosmeticModelBase;
...
public class Halo extends Cosmetic {
    ...
    public void render(AbstractClientPlayer player, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float headYaw, float headPitch, float scale) {
        User cosmeticUser = Spicy.getINSTANCE().getUser(player.getNameClear());
        if (cosmeticUser.hasHalo()) {
            GL11.glPushMatrix();
            playerRenderer.bindTexture(Spicy.getINSTANCE().cosmeticTextures.HALO);
    ...
    private class ModelHalo extends CosmeticModelBase {
        ...
        public void render(Entity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float headYaw, float headPitch, float scale) {
            ...
            ModelRenderer modelrenderer = bindTextureAndColor(Color.YELLOW, Spicy.getINSTANCE().cosmeticTextures.HALO, this.halo, null);
